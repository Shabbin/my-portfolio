...
const ServiceCard = ({ index, title, icon }) => {
  ...
  return isMobile ? (
    ...
    <Tilt className="xs:w-[260px] w-full">{CardContent}</Tilt>
  );
};

const IconAndTitle = ({ title, icon }) => (
  <>
    <div className="w-14 h-14 rounded-2xl flex items-center justify-center bg-emerald-400/90">
        ...
        >
          ...
          <h2 className={styles.sectionHeadText}>
            <span className="relative inline-block">
              Overview<span style={{ color: ACCENT_HEX }}>.</span>
            </span>
          </h2>
        </motion.div>
